...
<div class="flex flex-col">
    ...
    <div>
        ...
        <div>
            <a href="{{ route('print_nasabah') }}" target="blank" class="mt-2 block w-40 focus:outline-none text-white text-center bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5  dark:bg-orange-600 dark:ho ver:bg-orange-700 dark:focus:ring-orange-900 mb-2">Print</a>
        </div>
        <!-- Main modal -->
        <div id="authentication-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
            <div class="relative w-full h-full max-w-md md:h-auto">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-sidebar
                    ...
                    <div class="px-6 py-6 lg:px-4">
                        ...
                        <form class="space-y-6" action="/dashboard/nasabah" method="POST">
                            ...
                            <div class="flex flex-row gap-4">
                                <label class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Status Warga Curug</label>
                                <div class="flex items-center space-x-2">
                                    <input type="radio" name="wargaCurug" value="true" class="text-blue-500 focus:ring-blue-500 dark:focus:ring-blue-500" checked>
                                    <label class="text-sm text-gray-900 dark:text-white">Warga Curug</label>

                                    <input type="radio" name="wargaCurug" value="false" class="text-red-500 focus:ring-red-500 dark:focus:ring-red-500">
                                    <label class="text-sm text-gray-900 dark:text-white">Bukan Warga Curug</label>
                                </div>
                            </div>
                            <div class="flex flex-row gap-4">
                                <div>
                                ...
                                <div>
                                    <label for="small-input" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">No.
                                        HP</label>
                            ...
                            <label for="address" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Alamat</label>
                            ...
                            <div class="flex flex-row gap-4">
                                ...
                                <div>
                                    <label for="type" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kategori</label>
                            ...
                            <button type="submit" onclick="this.disabled=true; this.form.submit();" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Daftarkan</button>
...
<div class="w-full mt-10 bg-white border border-gray-200 rounded-lg shadow dark:bg-sidebar
    ...
    <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 rounded-t-lg bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-sidebar
        ...
        <li class="mr-2">
            <button id="about-tab" data-tabs-target="#about" type="button" role="tab" aria-controls="about" aria-selected="true" class="inline-block p-4 text-blue-600 rounded-tl-lg hover:bg-gray-100 dark:bg-sidebar
                     dark:hover:bg-gray-700 dark:text-blue-500">Nasabah</button>
        </li>
        <li class="mr-2">
            <button id="services-tab" data-tabs-target="#services" type="button" role="tab" aria-controls="services" aria-selected="false" class="inline-block p-4 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300">Pengepul</button>
        </li>
        <li class="mr-2">
            <button id="statistics-tab" data-tabs-target="#statistics" type="button" role="tab" aria-controls="statistics" aria-selected="false" class="inline-block p-4 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300">Admin</button>
        </li>
    </ul>
                ...
                <div class="overflow-auto max-h-[300px]">
                    <table class="w-full bg-white">
                        <thead class="bg-sidebar
                             text-white w-full">
                            <tr>
                            <th class="w-1/3 sm:w-auto text-left py-3 px-4 uppercase font-semibold text-sm">ID Member
                                </th>
                        ...
                        <tbody class="text-gray-700">
                            ...
                            <template x-if="searchText === '' || '{{ strtolower($user->name) }}'.includes(searchText.toLowerCase()) || '{{ $user->phone_number }}'.includes(searchText) || '{{ $user->id_member }}'.includes(searchText)">
                                <tr>
                                <td class="w-1/3 sm:w-auto text-left py-3 px-4">{{ $user->id_member }}</td>
                                    <td class="w-1/3 sm:w-auto text-left py-3 px-4">{{ $user->name }}</td>

                                    @if (isset(
                                    $user->transactions()->latest('created_at')->first()->created_at))
                                    @if (strtotime(
                                    $user->transactions()->latest('created_at')->first()->created_at) < strtotime('-3 months')) <td class="w-1/3 sm:w-auto bg-yellow-700 text-white text-left py-3 px-4">Aktif
                                        {{ $user->transactions()->latest('created_at')->first()->created_at->diffForHumans() }}
                                        </td>
                                        @elseif(strtotime(
                                        $user->transactions()->latest('created_at')->first()->created_at) < strtotime('-2 months')) <td class="w-1/3 sm:w-auto bg-orange-600 text-white text-left py-3 px-4">
                                            Aktif
                                            {{ $user->transactions()->latest('created_at')->first()->created_at->diffForHumans() }}
                                            </td>
                                            @else
                                            ...
                                            @endif
                                            @else
                                            @if (strtotime($user->created_at) < strtotime('-3 months')) <td class="w-1/3 sm:w-auto bg-red-700 text-left text-white py-3 px-4">Belum
                                                Melakukan Transaksi dari {{ $user->created_at->diffForHumans() }}</td>
                                                @elseif(strtotime($user->created_at) < strtotime('-2 months')) <td class="w-1/3 sm:w-auto bg-Purpel-700 text-left text-white py-3 px-4">
                                                    ...
                                                    <td class="sm:text-left py-3 px-4">
                                                        <a href="{{ route('nasabah_detail', ['user_id' => $user->id]) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Detail</a>
                                                    </td>
                                </tr>
                            </template>
                            @endforeach
